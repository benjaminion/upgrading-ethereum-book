...
It is expected that the protocol will get updated/upgraded from time to time, a process commonly known as a "hard-fork". For example, the upgrade from Phase&nbsp;0 to Altair took place on the 27th of October 2021. Unlike Eth1[^fn-eth1-forkid], Eth2 has an in-protocol concept of a version number. This is used, for example, to prevent votes from validators on one fork (that maybe haven't upgraded yet) being counted on a different fork.

[^fn-eth1-forkid]: Ethereum 1.0 has a fork identifier as defined in [EIP-2124](https://eips.ethereum.org/EIPS/eip-2124) which is similar, but the Eth1 fork id is not part of the consensus protocol, only the [networking protocol](https://eips.ethereum.org/EIPS/eip-2364).
...
> This is a digest primarily used for domain separation on the p2p layer. 4-bytes suffices for practical separation of forks/chains.
...
How do we know which chain we're on, and where we are on it? This information ought to be sufficient. A continuous path back to the [genesis block]() would also suffice.
